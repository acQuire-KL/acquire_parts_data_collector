...
def main() -> int:
    parser = argparse.ArgumentParser(description="Promote completed candidate reviews into engineering knowledge artefacts.")
    ...
    print("PDC Knowledge Promotion")
    print("=" * 40)
    print(f"Review file : {Path(args.review_file)}")
    for name, path in paths.items():
        print(f"{name.replace('_', ' ').title():22}: {path}")
    print("\nNo Parts Master was modified and no AIPNs were allocated.")
    return 0
